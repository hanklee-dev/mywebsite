...
def update_gist(file_name, content):
    ...
    if response.status_code == 200:
        print(f'{file_name}이(가) Gist에 성공적으로 업데이트되었습니다.')
    else:
        print(f'Gist 업데이트 실패: {response.status_code}')
        print(response.json())

# 로컬 파일 업데이트 함수
def update_local_file(file_name, content):
    with open(file_name, 'w', encoding='utf-8') as file:
        file.write(content)
    print(f'{file_name}이(가) 로컬에 성공적으로 업데이트되었습니다.')
...
# Gist와 로컬 파일 동시에 업데이트
file_name = 'index.html'
update_gist(file_name, new_content)
update_local_file(file_name, new_content)
